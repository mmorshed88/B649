		...
		throws IOException, InterruptedException {
			...
			if (rrd.targetUrlsList.size()<=0){
				...
				context.write(new LongWritable(i), new Text(String.valueOf(rankValuePerUrl)));
                         ...
                         double rankpertargeturl = rrd.rankValue/(double)rrd.targetUrlsList.size();
//                                for (int j=1;j<=rrd.targetUrlsList.size();j++){
             //        context.write(new LongWritable(rrd.targetUrlsList.get(j)), new Text(rankpertargeturl.toString()));
                                  for (int url: rrd.targetUrlsList){
                     context.write(new LongWritable(url), new Text(String.valueOf(rankpertargeturl)));
                     sb.append("#");
                     String temp=String.valueOf(url);
                     sb.append(temp);
                                      }
			} //for
                        if (rrd.targetUrlsList.size() > 0){
